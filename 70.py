...
def p67():
    """
    Find the maximal path total from top to bottom in `p067_triangle.txt`.
    """
    with open("p067_triangle.txt", "r") as f:
        triangle = [[int(x) for x in line.split(" ")] for line in f.readlines()]

    best = triangle.pop(-1)
    for row in reversed(triangle):
        best = [max(a + best[i], a + best[i + 1]) for i, a in enumerate(row)]
