...
def p51():
    ...
    for p in primegen:
        for replaceNo in range(1, 4):
            for places in itertools.combinations(list(range(len(str(p)))), replaceNo):
                family = set()
...
def pokerhand(hand):
    ...
    numbers = list(Counter(value[h[0]] for h in hand).items())
...
def p58():
    ...
    while numprimes / (4 * n + 1) >= .1:  # 4n+1 is number of entries on main diagonals
        n += 1
        corners = [
            (2 * n + 1) ** 2 - x * n for x in (2, 4, 6)
        ]  # last corner is square numbers
...
def xor(a, b):
    ...
    """
    return "".join(
        str(x) for x in (int(bool(int(i)) != bool(int(j))) for i, j in zip(a, b))
    )
...
def p60(n=10000, k=5):
    ...
    while changes:  # Trim the graph s.t. only the clique remains (its p sparse)
        changes = False
        for v in list(pairGraph.keys()):
            newneighbors = {
                n
                for n in pairGraph[v]
                if len(pairGraph.get(n, set()) & pairGraph[v]) >= k - 2
            }
